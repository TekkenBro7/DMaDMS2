...
BEGIN
  ...
  IF v_query_type = 'SELECT' THEN
    ...
    p_message := 'Running SELECT query.';
    p_rows    := 0;
  ...
  ELSIF v_query_type IN ('INSERT', 'UPDATE', 'DELETE') THEN
    ...
    p_message := 'DML operation ' || v_query_type || ' completed.';
    p_cursor  := NULL;
  ...
  ELSIF v_query_type = 'DDL' THEN
    ...
    ELSIF v_ddl_command = 'DROP TABLE' THEN
      ...
      EXECUTE IMMEDIATE v_query;
      p_message := 'Table ' || v_table || ' deleted.';
      p_rows    := 0;
      p_cursor  := NULL;
    ELSE
  ...
  ELSE
    RAISE_APPLICATION_ERROR(-20001, 'Wrong query type: ' || v_query_type);
  END IF;
