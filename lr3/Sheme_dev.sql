...
/
drop function dev.getreward1;

CREATE OR REPLACE NONEDITIONABLE FUNCTION "DEV"."GETREWARD1" (salary in number, percent in number)
return double precision
is
    ret double precision;
begin
...
CREATE INDEX dev.indx_new_value ON dev.NewTable(NewValue);
/

-- Пакет для схемы dev
CREATE OR REPLACE PACKAGE dev.ClientManagement AS
    PROCEDURE AddClient(
...
CREATE TABLE dev.STUDENTS (
    ...
    NAME VARCHAR2(100) NOT NULL,        
    GROUP_ID NUMBER,                    
    FOREIGN KEY (GROUP_ID) REFERENCES dev.GROUPS(ID)
);