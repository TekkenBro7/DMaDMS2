...
begin
    -- Если есть в dev, но нет в pron функции
...
begin
    ...
    with table_hierarchy as (
        ...
        from (
            ...
            where constraint_type = 'R' and owner = 'DEV'
        ) join (
            select owner parent_owner, constraint_name, table_name parent_table
            from all_constraints
            where constraint_type = 'P' and owner = 'DEV'
...
/
call cmp_pkg('prod', 'dev');
/
call cmp_tbl('prod', 'dev');
/

ALTER TABLE PROD.MYTABLE DROP CONSTRAINT EQE;

ALTER TABLE "PROD"."MYTABLE" ADD CONSTRAINT "UQ_VAL" UNIQUE ("VAL")
  USING INDEX PCTFREE 10 INITRANS 2 MAXTRANS 255 
  TABLESPACE "USERS"  ENABLE;

ALTER TABLE "PROD"."MYTABLE" ADD CONSTRAINT "CHK_VAL" CHECK (val > 0) ENABLE;

SELECT constraint_name, constraint_type, search_condition
FROM all_constraints
WHERE table_name = 'MYTABLE' AND owner = 'PROD';

SELECT constraint_name, constraint_type, search_condition
FROM all_constraints
WHERE table_name = 'MYTABLE' AND owner = 'DEV';
...
alter table PROD.MYTABLE drop constraint ID_PK;
alter table PROD.MYTABLE drop constraint UQ_VAL;

alter table PROD.TASKTYPE drop column SOMEFIELD;
alter table PROD.MYTABLE drop column ZZZ;

ALTER TABLE "PROD"."TST_C1" ADD CONSTRAINT "FK_C2_ID" FOREIGN KEY ("ID1")
	  REFERENCES "PROD"."TST_C2" ("ID") ENABLE;
      
alter table PROD.TST_C1 drop constraint FK_C2_ID;
...
CREATE OR REPLACE NONEDITIONABLE PROCEDURE "PROD"."I_DONT_PASS" 
( del_id23 in number )
is
begin
    delete from dev.MyTable where id=2;
end;