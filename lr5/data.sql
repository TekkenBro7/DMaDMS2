...
CREATE TABLE c##myuser.orders (
    ...
    CONSTRAINT fk_orders_customer FOREIGN KEY (customer_id) REFERENCES c##myuser.customers (customer_id)
);

CREATE TABLE c##myuser.order_items (
    ...
    CONSTRAINT fk_order_items_order FOREIGN KEY (order_id) REFERENCES c##myuser.orders (order_id)
);

CREATE TABLE c##myuser.audit_log (
...
CREATE OR REPLACE TRIGGER c##myuser.trg_customers_audit
BEFORE INSERT OR UPDATE OR DELETE ON c##myuser.customers
FOR EACH ROW
BEGIN
    IF INSERTING THEN
        INSERT INTO c##myuser.audit_log (table_name, pk_value, changed_data, operation_type)
        VALUES (
          'CUSTOMERS',
...
CREATE OR REPLACE TRIGGER c##myuser.trg_orders_audit
BEFORE INSERT OR UPDATE OR DELETE ON c##myuser.orders
FOR EACH ROW
BEGIN
    IF INSERTING THEN
        INSERT INTO c##myuser.audit_log (table_name, pk_value, changed_data, operation_type)
...
CREATE OR REPLACE TRIGGER c##myuser.trg_order_items_audit
BEFORE INSERT OR UPDATE OR DELETE ON c##myuser.order_items
FOR EACH ROW
BEGIN
    IF INSERTING THEN
        INSERT INTO c##myuser.audit_log (table_name, pk_value, changed_data, operation_type)
